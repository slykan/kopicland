<form wire:submit="search" class="grid gap-3 rounded-2xl bg-white p-4 shadow-lg sm:grid-cols-2 lg:grid-cols-6 lg:items-end">
    ...
    <div>
        ...
        <input type="number" min="1" wire:model="adults" class="mt-1 w-full rounded-lg border-brand-200 text-sm focus:border-brand-500 focus:ring-brand-500">
    </div>

    <div>
        <label class="block text-xs font-medium text-brand-600">{{ __('site.common.children') }}</label>
        <input type="number" min="0" wire:model="children" class="mt-1 w-full rounded-lg border-brand-200 text-sm focus:border-brand-500 focus:ring-brand-500">
    </div>
